                       ...
                       struct face *face, struct content_entry *content)
{
    ...
    if (h->logfile != -1){
        ...
        struct ccn_parsed_ContentObject tmpObj = {0};
        time_t rawtime;
        struct tm *timeinfo;
        char *prefix;

        ccn_parse_ContentObject(content->ccnb, content->size, &tmpObj, comps);
        flatname = ccn_charbuf_create();

        ccn_uri_append(flatname, content->ccnb, content->size, CCN_URI_INCLUDESCHEME);
        prefix = ccn_charbuf_as_string(flatname);

        time(&rawtime);
        timeinfo = localtime(&rawtime);

        if (ccn_is_final_pco(content->ccnb, &tmpObj, comps) == 1 &&
                (strstr(prefix, "%C1.META.M/.header") == NULL) &&
                (strstr(prefix, "%C1.R.sw") == NULL) &&
                (strstr(prefix, "ccnx:/%") == NULL) &&
                (strstr(prefix, "ccnx:/ccnx.org") == NULL)) {

            dprintf(h->logfile,
                    "%04d%02d%02d%02d%02d%02d;%s;%d;%u\n",
                    timeinfo->tm_year + 1900, timeinfo->tm_mon + 1, timeinfo->tm_mday,
                    timeinfo->tm_hour, timeinfo->tm_min, timeinfo->tm_sec,
                    prefix, h->content_tree->n, h->capacity);
        }
